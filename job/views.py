...
def recuiter_all(request):
    ...
    d = {'data': data}
    return render(request, 'recuiter_all.html', d)
...
def add_job(request):
    ...
    if request.method=='POST':
        ...
        recuiter = Recuiter.objects.get(user=user)
        try:
            Job.objects.create(recuiter=recuiter,end_date=ed,start_date=st,title=jt,salary=sal,image=l,experience=exc,location="Pending")
            error="no"
        except:
            error="yes"
    d={'error':error}
...
def job_list(request):
    ...
    user = request.user
    recuiter = Recuiter.objects.get(user=user)
    data = Job.objects.filter(recuiter=recuiter)
    d = {'data':data}
...
def apply_now(request,id):
    ...
    if request.method=='POST':
        ...
        try:
            ...
            data.location="Accepted"
            data.save()
            error="no"
        except:
            error="yes"
    d={'user':user,'error':error,'data':data}
...
def my_items(request):
    data = Applyjob.objects.all()
    d = {'data':data}
    return render(request,'my_items.html',d)
